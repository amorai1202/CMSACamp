...
```{r}
#Proportion of on-base vs out vs home run
...
stand_onbase <- filter_Batting %>% 
  group_by(stand, events) %>% 
  summarize(joint_prob = n() / nrow(filter_Batting), .groups = "drop") %>% 
  slice(c(1,4))

t.test(stand_onbase$joint_prob)

#Proportion of outs
stand_outs <- filter_Batting %>% 
  group_by(stand, events) %>% 
  summarize(joint_prob = n() / nrow(filter_Batting), .groups = "drop") %>% 
  slice(c(2,5))


t.test(stand_outs$joint_prob)

#Proportion of home-runs

stand_hr <- filter_Batting %>% 
  group_by(stand, events) %>% 
  summarize(joint_prob = n() / nrow(filter_Batting), .groups = "drop") %>% 
  slice(c(3,6))

t.test(stand_hr$joint_prob)
```

*** At alpha level 0.05, we have insufficient evidence to reject the null hypothesis that the proportion of outs between left and right-handed batters are different.***
